...
def how_many_medals_by_country(df, name):
	...
	d = df[df.Team == name]
	d = d.loc[(d.duplicated(subset=['Sport'], keep='first') == False) | (~df['Sport'].isin(mask))]
	d = d.drop(columns = ["ID","Name","Sex","Age","Height","Weight","Team","NOC","Games","Season","City","Event"]).fillna('None').sort_values(by=['Year'])
	if d.empty:
		return None
	dic = {}
